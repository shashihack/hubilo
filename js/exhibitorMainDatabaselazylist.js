    ...
    function search(result_page,search_in){        
        pagenum=0;
        search_value=$('#exhibitordatabase_main_search').val();
        sort_by_id=$('#exhibitor_sort_by').val();
        sort_filter_id=$('#exhibitor_filter_by').val();
    ...
    function showMore(id){
      $('#exhibitor-description-modal').modal();
      $('#modal-exhibitor-desc span').text(id);
    }

$(document).ready(function() {
    $('#exhibitor-grid').click(function(e){
        $("#exhibitor_sort_by").val("default");
        $("#exhibitor_filter_by").val("default");
        $("#exhibitordatabase_main_search").val("");
        pagecount=0;
        pagenum=0;
        search_value="";
        active = false;
        ...
        filter_eventtype="";
        sort_by_id="";
        $('#exhibitor_database_main').empty();
        setListName("#exhibitor_database_main");
        setResultPageName('getexhibitorMainDatabase_json.php');
        getresult('getexhibitorMainDatabase_json.php',0,'','#exhibitor_database_main','','','');
    });
});
